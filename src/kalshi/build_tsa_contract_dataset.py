"""Build a point-in-time-safe contract-level TSA dataset for model training."""
...
def _source_file_checksums() -> Dict[str, Optional[str]]:
    """Return checksums for core source files used for dataset generation."""
...
def build_contract_dataset(
    ...
    """Build one row per contract per event using only data available as of run_date."""
    ...
    for event_ticker in events:
        ...
        if pd.isna(pred_passengers):
            continue

        actual_ts = pd.Timestamp(event_date)
        if actual_ts not in passenger_data.index:
            continue
        actual_passengers = float(passenger_data.loc[actual_ts]["passengers_7_day_moving_average"])
        if pd.isna(actual_passengers):
            continue
        ...
        for market in event.get("markets", []):
            ...
            feature_row = contract_probability_model.build_contract_feature_row(
            ...
            rows.append(
                {
                    ...
                    "feature_max_source_date": filtered.index.max().date().isoformat(),
                    "actual_passengers": actual_passengers,
                    "y_yes_win": int(actual_passengers >= float(floor_strike)),
                    "floor_strike": float(floor_strike),
...
def write_dataset_and_metadata(
    ...
    metadata = {
        ...
        "label": "y_yes_win",
        "label_definition": "1 if actual_passengers >= floor_strike else 0",
    }
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build contract-level TSA training dataset.")
    parser.add_argument("--start", required=True, help="Start date YYYY-MM-DD")
    parser.add_argument("--end", required=True, help="End date YYYY-MM-DD")
    parser.add_argument("--out-csv", type=Path, default=DEFAULT_DATASETS / "tsa_contract_dataset.csv")
...
def main() -> None:
    ...
    dataset = build_contract_dataset(start_date=start_date, end_date=end_date)
